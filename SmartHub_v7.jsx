  ...
  return(
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.5)",zIndex:100,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
      <div style={{background:CARD,borderRadius:"20px 20px 0 0",padding:"20px 16px 28px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <button onClick={()=>setShared(s=>!s)} style={{display:"flex",alignItems:"center",gap:8,background:BG,border:"1.5px solid "+BORDER,borderRadius:10,padding:"8px 12px",cursor:"pointer"}}>
          <span style={{fontSize:14}}>{shared?"\ud83d\udc65":"\ud83d\udd12"}</span>
          <span style={{fontSize:12,color:T1}}>{shared?"Delad med hush\u00e5llet":"Bara f\u00f6r mig"}</span>
        </button>
...
function WidgetTodo({todos,onToggle,onAdd,onDelete,onToggleShared,compact,showPrivate}){
  const[text,setText]=useState("");const[addShared,setAddShared]=useState(true)
  const displayTodos=showPrivate?todos:todos.filter(t=>t.shared!==false)
  const remaining=displayTodos.filter(i=>!i.done).length
  function handleAdd(e){e.preventDefault();if(!text.trim())return;onAdd(text.trim(),addShared);setText("")}
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",gap:6}}>
      ...
      {onAdd&&(<div style={{display:"flex",gap:6,flexShrink:0,alignItems:"center"}}>
        <input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAdd(e)} placeholder="L\u00e4gg till..." style={{...inputStyle,padding:"6px 8px",fontSize:compact?10:12,flex:1}}/>
        {showPrivate&&<button onClick={()=>setAddShared(s=>!s)} style={{...btnSmall,fontSize:12,padding:"4px 6px",background:BG,borderRadius:8,border:"1px solid "+BORDER}} title={addShared?"Delad":"Privat"}>{addShared?"\ud83d\udc65":"\ud83d\udd12"}</button>}
        <button onClick={handleAdd} style={{background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"0 12px",fontSize:13,fontWeight:600,cursor:"pointer",flexShrink:0,opacity:text.trim()?1:0.4,height:30}}>+</button>
      </div>)}
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:5,overflowY:"auto"}}>
        {displayTodos.length===0&&<span style={{fontSize:10,color:T3,fontStyle:"italic"}}>Inga uppgifter</span>}
        {displayTodos.map(item=>(<div key={item.id} style={{display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
          ...
          {item.shared===false&&<span style={{fontSize:9,color:T3}} title="Privat">{"\ud83d\udd12"}</span>}
          {onToggleShared&&<button onClick={()=>onToggleShared(item)} style={{...btnSmall,fontSize:10,color:T3}}>{item.shared?"\ud83d\udc65":"\ud83d\udd12"}</button>}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:8,overflowY:"auto"}}>
        ...
        {eventsToday.map(e=>(<div key={e.id} style={{display:"flex",gap:8,alignItems:"flex-start",flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:T1,fontWeight:500}}>{e.title}{e.shared===false&&<span style={{fontSize:8,color:T3,marginLeft:4}}>{"\ud83d\udd12"}</span>}</div>
...
function WidgetVader({weather}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:4}}>
    <span style={{fontSize:32}}>{weather.icon}</span><span style={{fontSize:28,fontWeight:200,color:T1}}>{weather.temp}{"\u00b0"}</span><span style={{fontSize:10,color:T3}}>{weather.desc} {"\u00b7"} {weather.location}</span><span style={{fontSize:9,color:T3}}>Vind: {weather.wind} m/s</span>
    <div style={{display:"flex",gap:10,marginTop:6}}>{weather.forecast.map(w=>(<div key={w.day} style={{textAlign:"center"}}><div style={{fontSize:9,color:T3}}>{w.day}</div><div style={{fontSize:14}}>{w.icon}</div><div style={{fontSize:10,color:T2,fontWeight:500}}>{w.temp}</div></div>))}</div>
  </div>)
}
...
function renderWidget(id,ctx){
  switch(id){
    ...
    case"todo":return<WidgetTodo todos={ctx.todos} onToggle={ctx.onToggleTodo} onAdd={ctx.onAddTodo} onDelete={ctx.onDeleteTodo} compact showPrivate={false}/>
    ...
    case"vader":return<WidgetVader weather={ctx.weather}/>
  ...
  return(
    <div style={{...bg.style,padding:"20px 20px 14px",position:"relative",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{textAlign:"right"}}><div style={{fontSize:26}}>{wi.icon}</div><div style={{fontSize:20,fontWeight:200,color:tc,textShadow:ts}}>{wi.temp}{"\u00b0"}</div><div style={{fontSize:10,color:tc,opacity:0.7}}>{wi.desc}</div></div>
      </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      ...
      {onDeleteEvent&&(<div style={{background:CARD,borderRadius:14,border:"0.5px solid "+BORDER,padding:14}}>
        ...
        {events.filter(e=>new Date(e.start_time)>=new Date(new Date().setHours(0,0,0,0))).slice(0,15).map(ev=>(<div key={ev.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"0.5px solid "+BORDER}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:10,color:T3}}>{new Date(ev.start_time).toLocaleDateString("sv-SE")} {fmtTime(ev.start_time)}</div><div style={{fontSize:12,color:T1,fontWeight:500}}>{ev.title}{ev.shared===false&&<span style={{fontSize:8,color:T3,marginLeft:4}}>{"\ud83d\udd12"}</span>}</div></div>
...
function SettingsTab({householdId,userId,householdName}){
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",display:"flex",flexDirection:"column",gap:12}}>
      <div style={{background:CARD,borderRadius:14,border:"0.5px solid "+BORDER,padding:16}}>
        <div style={{fontSize:13,fontWeight:600,color:T1,marginBottom:12}}>Hush{"\u00e5"}ll: {householdName}</div>
        <div style={{fontSize:11,fontWeight:600,color:T3,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Medlemmar</div>
        {members.map((m,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:i<members.length-1?"0.5px solid "+BORDER:"none"}}>
          <div style={{width:28,height:28,borderRadius:"50%",background:ACCENT_SOFT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:ACCENT,fontWeight:600}}>{m.user_id===userId?"Du":String(i+1)}</div>
          <div style={{flex:1}}><div style={{fontSize:12,color:T1}}>{m.user_id===userId?"Du":"Medlem "+(i+1)}</div><div style={{fontSize:10,color:T3}}>{m.role}</div></div>
        </div>))}
      </div>

      <div style={{background:CARD,borderRadius:14,border:"0.5px solid "+BORDER,padding:16}}>
        <div style={{fontSize:11,fontWeight:600,color:T3,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:12}}>Bjud in</div>
        {!inviteCode?(
          <button onClick={createInvite} disabled={creating} style={{width:"100%",background:ACCENT,color:"#fff",border:"none",borderRadius:12,padding:"12px",fontSize:14,fontWeight:600,cursor:creating?"not-allowed":"pointer",opacity:creating?0.6:1}}>
            {creating?"Skapar kod...":"Skapa inbjudningskod"}
          </button>
        ):(
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:10,color:T3,marginBottom:8}}>Dela denna kod med din familjemedlem</div>
            <div onClick={copyCode} style={{fontFamily:"monospace",fontSize:32,fontWeight:700,letterSpacing:"0.15em",color:ACCENT,cursor:"pointer",padding:"12px",background:ACCENT_SOFT,borderRadius:12,marginBottom:8}}>{inviteCode}</div>
            <div style={{fontSize:10,color:copied?GREEN:T3}}>{copied?"Kopierad!":"Tryck f\u00f6r att kopiera \u00b7 Giltig i 7 dagar"}</div>
            <button onClick={()=>{setInviteCode(null)}} style={{marginTop:12,background:"none",border:"1px solid "+BORDER,borderRadius:10,padding:"8px 16px",fontSize:12,color:T2,cursor:"pointer"}}>Skapa ny kod</button>
          </div>
        )}
      </div>
    </div>
...
export default function SmartHub({session,household}){
  ...
  // ----- Weather -----
  useEffect(()=>{async function f(){try{const r=await fetch("https://api.open-meteo.com/v1/forecast?latitude="+WEATHER_LAT+"&longitude="+WEATHER_LON+"&current=temperature_2m,weathercode,windspeed_10m&daily=weathercode,temperature_2m_max&timezone=Europe/Stockholm&forecast_days=4");if(!r.ok)return;const json=await r.json();const parsed=parseWeather(json);if(parsed)setWeather(parsed)}catch(e){console.error("[weather]",e)}};f();const t=setInterval(f,30*60*1000);return()=>clearInterval(t)},[])
  ...
  async function handleAddTodo(text,shared=true){const tmp={id:"tmp-"+Date.now(),household_id:householdId,text,done:false,shared,created_by:userId,created_at:new Date().toISOString()};setTodos(p=>[...p,tmp]);const{data,error}=await supabase.from("todos").insert({household_id:householdId,text,done:false,shared,created_by:userId}).select().single();if(error){console.error("[todos]",error);setTodos(p=>p.filter(t=>t.id!==tmp.id))}else setTodos(p=>p.map(t=>t.id===tmp.id?data:t))}
  ...
  async function handleToggleSharedTodo(item){const ns=!item.shared;setTodos(p=>p.map(t=>t.id===item.id?{...t,shared:ns}:t));const{error}=await supabase.from("todos").update({shared:ns}).eq("id",item.id);if(error){console.error("[todos]",error);setTodos(p=>p.map(t=>t.id===item.id?item:t))}}
  ...
  const allEventsToday=useMemo(()=>getEventsToday(calEvents),[calEvents])
  const sharedTodos=todos.filter(t=>t.shared!==false)
  const widgetCtx={todos:sharedTodos,onToggleTodo:handleToggleTodo,onAddTodo:handleAddTodo,onDeleteTodo:handleDeleteTodo,calEventsByDay,eventsToday,onDeleteEvent:handleDeleteEvent,meals,onUpsertMeal:handleUpsertMeal,weather}
  ...
  return(
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:BG,display:"flex",flexDirection:"column",height:"100dvh",position:"relative",overflow:"hidden",zoom:2.25}}>
      ...
      <div style={{flex:1,padding:10,minHeight:0,overflow:"hidden"}}>
        ...
        {tab==="listor"&&<div style={{height:"100%",overflowY:"auto"}}><div style={{background:CARD,borderRadius:14,border:"0.5px solid "+BORDER,padding:16}}><WidgetTodo todos={todos} onToggle={handleToggleTodo} onAdd={handleAddTodo} onDelete={handleDeleteTodo} onToggleShared={handleToggleSharedTodo} showPrivate/></div></div>}
