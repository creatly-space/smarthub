...
function WidgetVader({weather}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:4}}>
    ...
    <span style={{fontSize:28,fontWeight:200,color:T1}}>{weather.temp}{"\u00b0"}</span>
    <span style={{fontSize:10,color:T3}}>{weather.desc} {"\u00b7"} {weather.location}</span>
    <span style={{fontSize:9,color:T3}}>Vind: {weather.wind} m/s</span>
    <div style={{display:"flex",gap:10,marginTop:6}}>
      {weather.forecast.map(w=>(<div key={w.day} style={{textAlign:"center"}}><div style={{fontSize:9,color:T3}}>{w.day}</div><div style={{fontSize:14}}>{w.icon}</div><div style={{fontSize:10,color:T2,fontWeight:500}}>{w.temp}</div></div>))}
    </div>
  </div>)
}
...
export default function SmartHub({session,household}){
  ...
  const widgetCtx={todos,onToggleTodo:handleToggleTodo,onAddTodo:handleAddTodo,onDeleteTodo:handleDeleteTodo,calEventsByDay,eventsToday,onDeleteEvent:handleDeleteEvent,meals,onUpsertMeal:handleUpsertMeal,weather}
